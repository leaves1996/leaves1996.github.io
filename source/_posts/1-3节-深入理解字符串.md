...
题目解析：String s1 = "hi," + "ye" + "xiaomo" 代码会被 JVM 优化为：String s1 = "hi,yexiaomo"，这样就和 s3 完全相同，s1 创建的时候会把字符"hi,yexiaomo"放入常量池，s3 创建的时候，常量池中已经存在对应的缓存，会直接把引用返回给 s3，所以 s1==s3 就为 true，而 s2 使用了 += 其引用地址就和其他两个不同。
